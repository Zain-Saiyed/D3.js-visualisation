...
// create a group to contain the radar chart in the chart-6 SVG container
var radar_chart_svg = chart6_svg
  .append("g")
  .attr("transform", "translate(" + (940) + "," + (1650) + ")"); // move it to desired location on web.
...
var angle_slice = Math.PI * 2 / potential_technology_data.length; // Divide 360 degree (2 pi) into quadrants each for each unique value in our dataset
...
radar_chart_svg.selectAll(".radar-label")
    ...
    .attr("x", function (d, i) { return radial_scale(40 + 18) * Math.cos(angle_slice * i - Math.PI / 2); })
...
radar_chart_svg.append("path")
  .datum([...potential_technology_data, Object.assign({}, potential_technology_data[0])]) // Adding first element at the end of the array, so as to get a closed figure, and not a open ended figure.
  .attr("class", "radar-path")
  .attr("d", line)
  .attr("fill", color_palette[6])
  .attr("stroke", secondary_text_color_accent)
  .attr("stroke-width", 2)
